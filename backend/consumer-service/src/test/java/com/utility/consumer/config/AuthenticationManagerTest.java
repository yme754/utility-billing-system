...
class AuthenticationManagerTest {
    ...
    @Test
    void authenticate_exception_returnsEmpty() {
        when(jwtUtil.validateToken("boom")).thenReturn(true);
        when(jwtUtil.getAllClaimsFromToken("boom")).thenThrow(new RuntimeException("fail"));
        Authentication input = new UsernamePasswordAuthenticationToken("user", "boom");
